...
class ChatServer(rpc.ChatServerServicer):  # inheriting here from the protobuf rpc file which is generated
    ...
    def __init__(self):
        ...
        self.chats = []
        self.registeredUsersdict = {}
        self.userchats = []

        # the function to store register details of the user
    ...
    def register(self, request: chat.registerdetails, context):
        print("[{}] {} {}".format(request.email, request.username, request.password))
        self.registeredUsersdict[request.username] = request.password
        return chat.Empty()

        # the function to check if user has valid details
